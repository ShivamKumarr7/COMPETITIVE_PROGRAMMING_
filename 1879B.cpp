...
int main()
{
    ...
    while(t--)
    {
        ...
        cin>>n;
        vector<ll>a(n,0);
        ll row_mini = LLONG_MAX;
        for(int i=0;i<n;i++)
        {
            cin>>a[i];
            row_mini = min(row_mini,a[i]);
        }
        ll col_mini = LLONG_MAX;
        vector<ll>b(n,0);
        for(int i=0;i<n;i++)
        {
            cin>>b[i];
            col_mini = min(col_mini,b[i]);
        }
        // debug(a)
        // debug(b)
        ll row_sum = 0;
        for(int i=0;i<n;i++)
        {
            row_sum += (a[i] + col_mini);
        }
        ll col_sum = 0;
        for(int i=0;i<n;i++)
        {
            col_sum += (b[i] + row_mini);
        }
        ll ans = min(row_sum,col_sum);
        cout<<ans<<"\n";
    }
    return 0;
}
